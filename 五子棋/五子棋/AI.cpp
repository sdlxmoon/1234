...
void AI::go()
{
    ...
    Chesspos pos1 = think();
	Sleep(1500);
	chess->Chessdown(&pos1, CHESS_White);
}

void AI::caculatesorce()
{
	...
	for (int row = 0; row < size; row++) {
		for (int col = 0; col < size; col++) {
			//对每个点进行计算
			if (chess->Chessdata(row, col)) continue;
				...
				for (int x = -1; x <= 1; x++) {    
					...
					for (int i = 1; i <= 4; i++) {
						...
						if (curRow >= 0 && curRow < size &&
							...
							chess->Chessdata(curRow, curCol) == 1) {
							personNum++;
						}
						else if (curRow >= 0 && curRow < size &&
							curCol >= 0 && curCol < size &&
							chess->Chessdata(curRow, curCol) == 0) {
							emptyNum++;
							break;
						}
						else {
